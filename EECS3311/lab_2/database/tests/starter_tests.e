...
feature
	make
		do
			...
			-- My own unit tests
			add_boolean_case (agent t_insert_and_count)
			add_boolean_case (agent t_insert_and_has_key)
			add_boolean_case (agent t_insert_and_search)
			add_boolean_case (agent t_delete)
			add_boolean_case (agent t_model)
...
feature -- unit tests - LINEAR_DB

	t_insert_and_count: BOOLEAN
	do
		comment ("t_insert_and_count: Test insert & count functionality")
		reset_as_linear_db
		Result := db_int_int.count = 0
		check Result end
		db_int_int.insert (1, 100)
		Result := db_int_int.count = 1
		check Result end
		db_int_int.insert (2, 200)
		Result := db_int_int.count = 2
		check Result end
	...
	t_insert_and_has_key: BOOLEAN
	do
		comment ("t_insert_and_has_key: Test insert & has_key functionality")
		reset_as_linear_db
		Result := db_int_int.has_key (1) = false
		check Result end
		db_int_int.insert (1, 100)
		Result := db_int_int.has_key (1) = true
		check Result end
	...
	t_insert_and_search: BOOLEAN
	do
		comment ("t_insert_and_search: Test insert & search functionality")
		reset_as_linear_db
		Result := db_int_int.has_key (1) = false
		check Result end
		db_int_int.insert (1, 100)
		Result := db_int_int.search (1) = 100
		check Result end
		db_int_int.insert (2, 200)
		Result := db_int_int.search (2) /= 100
		check Result end
		db_int_int.insert (3, 300)
		Result := db_int_int.search (3) = 300
		check Result end
	...
	t_delete: BOOLEAN
	do
		comment ("t_delete: Test delete functionality")
		reset_as_linear_db
		db_int_int.insert (1, 100)
		db_int_int.delete (1)
		Result := db_int_int.count = 0
		...
		db_int_int.insert (2, 200)
		db_int_int.insert (3, 300)
		db_int_int.insert (4, 400)
		Result := db_int_int.count = 4
		check Result end
		Result := db_int_int.has_key (1) = true
		check Result end
		db_int_int.delete (1)
		...
		Result := db_int_int.count = 2
		check Result end
		Result := db_int_int.has_key (3) = false
		check Result end
		db_int_int.delete (2)
		Result := db_int_int.count = 1
		check Result end
		Result := db_int_int.has_key (2) = false
		check Result end
		db_int_int.delete (4)
		Result := db_int_int.count = 0
		check Result end
		Result := db_int_int.has_key (4) = false
		check Result end
	...
	t_model: BOOLEAN
	local
		rel: REL[INTEGER, INTEGER]
		tuples: ARRAY[TUPLE[k: INTEGER; v: STRING]]
	do
		comment ("t_model: Test model sanity")
		reset_as_linear_db
		Result := db_int_int.has_key (1) = false
		check Result end
		db_int_int.insert (1, 100)
		...
		db_int_int.insert (3, 300)
		db_int_int.insert (4, 400)
		rel := db_int_int.model
		Result := rel.domain.count = 4
		check Result end
		Result := rel[1].count = 1 and rel[1].has (100)
		check Result end
		Result := rel[2].count = 1 and rel[2].has (200)
		check Result end
		Result := rel[3].count = 1 and rel[3].has (300)
		check Result end
		Result := rel[4].count = 1 and rel[4].has (400)
		check Result end
		Result := rel[0].count = 0
		check Result end
